...
export interface FileToolArgs {
  operation: 'read' | 'write' | 'list' | 'delete' | 'metadata' | 'undo' | 'redo' | 'history' | 'sync';
  filePath: string;
  content?: string;
  peerId?: string; // For distributed operations
}

/**
 * FileSystemTool - Pipeline-compatible file operations
 * Implements MCPTool interface for dynamic discovery
 */
export class FileSystemTool implements MCPTool {
  public readonly name = 'fs';
  public readonly description = 'File system operations with versioning, undo/redo, and distributed sync';
